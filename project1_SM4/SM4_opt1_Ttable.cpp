...
// 优化 T 可算表
//unsigned long T0[256], T1[256], T2[256], T3[256];
...
void init_T_tables() {
    for (int x = 0; x < 256; x++) {
        unsigned char s = sm4Sbox(x);
        T0[x] = L((unsigned long)s << 24);
        T1[x] = L((unsigned long)s << 16);
        T2[x] = L((unsigned long)s << 8);
        T3[x] = L((unsigned long)s);
    }
}
*/
unsigned long T_opt(unsigned long x) {
...
void key_expansion(unsigned long MK[4], unsigned long rk[32]) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        unsigned long Lval = b ^ (b << 13 | b >> (32 - 13)) ^ (b << 23 | b >> (32 - 23));
        rk[i] = K[i] ^ Lval;
        K[i + 4] = rk[i];
    }
}
...
unsigned long round_operate(int i, unsigned long* X, unsigned long* rk) {
    return X[i] ^ T_opt(X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i]);
}
...
void sm4_dec(unsigned long MK[4], unsigned long X[4]) {
    ...
    for (int i = 0; i < 32; i++) tmpX[i + 4] = round_operate(i, tmpX, rk);
...
int main() {
    ...
    unsigned long X[36] = { 0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210 };
    unsigned long C[36] = { 0x681edf34, 0xd206965e, 0x86b3e94f, 0x536e4246 };
