...
unsigned char sm4Sbox(unsigned char in) {
    return SboxTable[(in >> 4) & 0x0F][in & 0x0F];
}


// SIMD 优化的 4个32位整数循环左移
inline __m128i rotl32_simd(__m128i x, int r) {
    return _mm_or_si128(_mm_slli_epi32(x, r), _mm_srli_epi32(x, 32 - r));
}

// SIMD优化L变换：对4个32位整数同时做L变换
inline __m128i L_simd(__m128i x) {
    __m128i t1 = rotl32_simd(x, 2);
    __m128i t2 = rotl32_simd(x, 10);
    __m128i t3 = rotl32_simd(x, 18);
    __m128i t4 = rotl32_simd(x, 24);
    return _mm_xor_si128(_mm_xor_si128(_mm_xor_si128(x, t1), t2), _mm_xor_si128(t3, t4));
}
...
unsigned long T(unsigned long x) {
    ...
    __m128i val = _mm_set_epi32(0, 0, 0, b); 
    __m128i res = L_simd(val);
...
void key_expansion(unsigned long MK[4], unsigned long rk[32]) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        __m128i b_rot13 = rotl32_simd(b_vec, 13);
        __m128i b_rot23 = rotl32_simd(b_vec, 23);
        __m128i result = _mm_xor_si128(_mm_xor_si128(b_vec, b_rot13), b_rot23);
        unsigned long Lval = _mm_cvtsi128_si32(result); 
...
int main() {
    ...
    auto duration_enc = chrono::duration_cast<chrono::microseconds>(end_enc - start_enc).count();
    cout << "加密耗时：" << duration_enc << " 微秒" << endl;

    auto start_dec = chrono::high_resolution_clock::now();
    sm4_dec(MK, C);
    auto end_dec = chrono::high_resolution_clock::now();
    auto duration_dec = chrono::duration_cast<chrono::microseconds>(end_dec - start_dec).count();
    cout << "解密耗时：" << duration_dec << " 微秒" << endl;
